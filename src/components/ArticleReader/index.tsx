...
export const ArticleReader: React.FC<Props> = React.memo((props) => {
  ...
  const getThemeStyles = (themeProp: UserTheme): ThemeStyles => {
  ...
  const getHtmlHeader = (themeStyle: ThemeStyles, textDirection: 'rtl' | 'ltr'): string => {
    return `
      <head>
  ...
  const getNoHtmlDocument = (articleProp: Api.Article | undefined, themeStyle: ThemeStyles): string => {
  ...
  const getTextDirection = (articleProp: Api.Article | undefined): TextDirection => {
